...
Every meeting produces notes, and the notes outlive the meeting. Decisions, owners, and open questions go in the doc while it's fresh, and the doc goes somewhere the whole company can find it. This is what makes it safe to not invite everyone. Nobody has to sit in a call defensively just to stay informed, because the record is public by default and they can catch up in five minutes.

<Callout type="note" title="If it wasn't written down">
  A decision that lives only in someone's memory of a call isn't a decision, it's a rumor. Write it down or assume it didn't happen.
</Callout>
...
We write documents, not presentations. A deck is optimized for the person presenting; a document is optimized for the person reading. Prose forces you to make an actual argument, with the reasoning in full sentences, instead of hiding behind six bullets and a chart you talk over. If your thinking survives being written out plainly, it's probably sound. If it only works with you narrating, it probably wasn't ready.
