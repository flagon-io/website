...
If the underlying model is wrong, no amount of polish saves it. And if the model is right, a plain version of it already works. Get the bones right first.
...
An empty state is a chance to teach instead of a blank shrug. An error message is a chance to tell the truth plainly instead of hiding behind "something went wrong." The first-run experience is the whole first impression, made once. Spend real attention there, because that's where taste actually shows up. This is the same idea as [have a point of view](/handbook/have-a-point-of-view): character comes from caring about the parts most people skip.

<Callout type="brand" title="The finish test">
  If a competitor could ship the same screen and nobody would blink, it isn't
  done yet. The details are where it becomes unmistakably ours.
</Callout>
